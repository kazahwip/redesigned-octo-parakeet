...
def create_payment_method_keyboard(
    ...
    if payment_methods and payment_methods.get("card_transfer"):
        builder.button(text="💳 Перевод на карту", callback_data="pay_card_transfer")
...
def create_topup_payment_method_keyboard(payment_methods: dict) -> InlineKeyboardMarkup:
    ...
    if payment_methods and payment_methods.get("card_transfer"):
        builder.button(text="💳 Перевод на карту", callback_data="topup_pay_card_transfer")
    if payment_methods and payment_methods.get("cryptobot"):
...
def create_card_transfer_paid_keyboard(payment_id: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Я перевел(а), проверить", callback_data=f"card_transfer_paid_{payment_id}")
    return builder.as_markup()


def create_card_transfer_admin_keyboard(payment_id: str, user_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Подтвердить", callback_data=f"admin_card_confirm_{payment_id}")
    builder.button(text="❌ Отклонить", callback_data=f"admin_card_reject_{payment_id}_{user_id}")
    builder.adjust(2)
    return builder.as_markup()
