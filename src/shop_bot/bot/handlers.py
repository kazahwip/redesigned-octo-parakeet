...
from shop_bot.data_manager.database import (
    ...
    mark_pending_transaction_review,
...
def get_user_router() -> Router:
    ...
    async def topup_pay_yoomoney(callback: types.CallbackQuery, state: FSMContext):
        await callback.answer()
        await callback.message.edit_text("❌ Оплата через YooMoney отключена.")
        await state.clear()
        return
    ...
    @user_router.callback_query(PaymentProcess.waiting_for_payment_method, F.data == "pay_card_transfer")
    async def create_card_transfer_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        await callback.answer("Готовлю реквизиты для перевода...")
        data = await state.get_data()
        user_data = get_user(callback.from_user.id)
        plan = get_plan_by_id(data.get('plan_id'))
        if not plan:
        ...
        enabled = str(get_setting("card_transfer_enabled") or "").lower() in ("true", "1", "yes", "on")
        card_number = (get_setting("card_number") or "").strip()
        if not enabled or not card_number:
            await callback.message.edit_text("❌ Оплата переводом на карту временно недоступна.")
            await state.clear()
            return
        ...
        metadata = {
            ...
            "plan_id": data.get('plan_id'),
            "customer_email": data.get('customer_email'),
            "payment_method": "CardTransfer",
            "promo_code": data.get('promo_code'),
            "promo_discount_percent": data.get('promo_discount_percent'),
            "promo_discount_amount": data.get('promo_discount_amount'),
        }
        try:
            create_pending_transaction(payment_id, callback.from_user.id, float(final_price_decimal), metadata)
        except Exception as e:
            logger.warning(f"Card transfer: не удалось создать ожидающую транзакцию: {e}")

        card_holder = (get_setting("card_holder") or "").strip()
        card_bank = (get_setting("card_bank") or "").strip()
        instructions = (get_setting("card_transfer_instructions") or "").strip()
        lines = [
            "💳 Оплата переводом на карту",
            "",
            f"Сумма к оплате: <b>{float(final_price_decimal):.2f} RUB</b>",
            f"Номер карты: <code>{card_number}</code>",
        ]
        if card_holder:
            lines.append(f"Получатель: <b>{card_holder}</b>")
        if card_bank:
            lines.append(f"Банк: <b>{card_bank}</b>")
        lines.extend([
            "",
            f"Назначение/комментарий к переводу: <code>{payment_id}</code>",
        ])
        if instructions:
            lines.extend(["", instructions])
        lines.extend([
            "",
            "После перевода нажмите кнопку ниже. Администратор проверит платеж и выдаст ключ.",
        ])

        await state.clear()
        await callback.message.edit_text(
            "\n".join(lines),
            reply_markup=keyboards.create_card_transfer_paid_keyboard(payment_id),
        )

    @user_router.callback_query(TopUpProcess.waiting_for_topup_method, F.data == "topup_pay_card_transfer")
    async def topup_pay_card_transfer(callback: types.CallbackQuery, state: FSMContext):
        await callback.answer("Готовлю реквизиты для перевода...")
        data = await state.get_data()
        amount_rub = Decimal(str(data.get('topup_amount', 0)))
        if amount_rub <= 0:
        ...
        instructions = (get_setting("card_transfer_instructions") or "").strip()
        lines = [
            "💳 Пополнение баланса переводом на карту",
            "",
            f"Сумма к оплате: <b>{float(amount_rub):.2f} RUB</b>",
            f"Номер карты: <code>{card_number}</code>",
        ]
        if card_holder:
            lines.append(f"Получатель: <b>{card_holder}</b>")
        ...
        lines.extend([
            "",
            "После перевода нажмите кнопку ниже. Администратор подтвердит зачисление.",
        ])
    ...
    @user_router.callback_query(F.data.startswith("card_transfer_paid_"))
    async def card_transfer_paid_handler(callback: types.CallbackQuery, bot: Bot):
        payment_id = callback.data[len("card_transfer_paid_"):].strip()
        if not payment_id:
        ...
        if not tx:
            ...
            return
        if int(tx.get("user_id") or 0) != int(callback.from_user.id):
        ...
        if status in ("paid", "success", "succeeded"):
            ...
            return
        if status == "rejected":
            await callback.answer("Платеж отклонен администратором.", show_alert=True)
            return
        if status == "pending_review":
            await callback.answer("Заявка уже отправлена администратору.", show_alert=True)
            return
        if status == "pending":
            mark_pending_transaction_review(payment_id, callback.from_user.id)
        ...
        if not admins:
            admin_single = get_setting("admin_telegram_id")
            if admin_single:
                admins = [admin_single]

        plan = get_plan_by_id(metadata.get("plan_id")) if metadata.get("plan_id") else None
        plan_name = plan.get("plan_name") if plan else "N/A"
        action = metadata.get("action") or "unknown"
        action_text = "Пополнение" if action == "top_up" else ("Новый ключ" if action == "new" else "Продление")
        text = (
            "📥 Запрос на проверку перевода на карту\n"
            f"Пользователь: {callback.from_user.id}\n"
            f"Сумма: {float(tx.get('amount_rub') or 0):.2f} RUB\n"
            f"Тип: {action_text}\n"
            f"Тариф: {plan_name}\n"
        ...
        try:
            await callback.answer("Заявка отправлена администратору на проверку.")
        except Exception:
            pass

    @user_router.callback_query(F.data.startswith("admin_card_confirm_"))
    async def admin_card_confirm_handler(callback: types.CallbackQuery, bot: Bot):
        if not is_admin(callback.from_user.id):
            await callback.answer("Недостаточно прав.", show_alert=True)
            return
        payment_id = callback.data[len("admin_card_confirm_"):].strip()
        if not payment_id:
        ...
        if status in ("paid", "success", "succeeded"):
            ...
            return
        if status == "rejected":
            await callback.answer("Платеж уже отклонен.", show_alert=True)
            return
        ...
        if not md:
            await callback.answer("Не удалось подтвердить платеж.", show_alert=True)
            return
        md["payment_id"] = payment_id
        await process_successful_payment(bot, md)

        user_id = _to_int_or_default(md.get("user_id"), 0)
        if user_id:
            try:
                await bot.send_message(user_id, "✅ Оплата подтверждена администратором.")
            except Exception:
                pass
        await callback.message.edit_text(f"✅ Платеж <code>{payment_id}</code> подтвержден.")
        await callback.answer("Подтверждено.")

    @user_router.callback_query(F.data.startswith("admin_card_reject_"))
    async def admin_card_reject_handler(callback: types.CallbackQuery, bot: Bot):
        if not is_admin(callback.from_user.id):
            await callback.answer("Недостаточно прав.", show_alert=True)
            return
        ...
        status = (tx.get("status") or "").lower()
        if status in ("paid", "success", "succeeded"):
            await callback.answer("Платеж уже подтвержден, отклонить нельзя.", show_alert=True)
            return
        update_transaction_status(payment_id, "rejected", "CardTransfer")

        if not user_id:
            user_id = _to_int_or_default(tx.get("user_id"), 0)
        if user_id:
            try:
                await bot.send_message(
                    user_id,
                    "❌ Платеж отклонен администратором. Проверьте реквизиты и обратитесь в поддержку.",
                    reply_markup=keyboards.create_support_keyboard(),
                )
            except Exception:
                pass

        await callback.message.edit_text(f"❌ Платеж <code>{payment_id}</code> отклонен.")
    ...
    async def create_cryptobot_invoice_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        final_price_decimal = price_rub_decimal
        try:
            final_price_from_state = data.get('final_price')
            if final_price_from_state is not None:
                final_price_decimal = Decimal(str(final_price_from_state)).quantize(Decimal("0.01"))
...
async def _create_cryptobot_invoice(
    ...
    try:
        ...
        create_pending_transaction(payment_id, int(user_id), float(price_rub), metadata)
...
async def process_successful_payment(bot: Bot, metadata: dict):
    try:
        action = metadata.get('action')
        user_id = _to_int_or_default(metadata.get('user_id'), 0)
        price = _to_float_or_default(metadata.get('price'), 0.0)
        if user_id <= 0:
            raise ValueError("invalid user_id")
        # Поля ниже нужны только для покупок ключей/продлений
        months = _to_int_or_default(metadata.get('months', 0), 0)
        key_id = _to_int_or_default(metadata.get('key_id', 0), 0)
        host_name = metadata.get('host_name', '')
        plan_id = _to_int_or_default(metadata.get('plan_id', 0), 0)
