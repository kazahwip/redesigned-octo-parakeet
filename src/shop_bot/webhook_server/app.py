...
ALL_SETTINGS_KEYS = [
    ...
    "card_transfer_enabled", "card_number", "card_holder", "card_bank", "card_transfer_instructions",
...
def create_webhook_app(bot_controller_instance):
    ...
    def settings_page():
        if request.method == 'POST':
            ...
            checkbox_keys = ['force_subscription', 'sbp_enabled', 'trial_enabled', 'enable_referrals', 'enable_fixed_referral_bonus', 'stars_enabled', 'card_transfer_enabled', 'monitoring_enabled']
    ...
    def cryptobot_webhook_handler():
        try:
            ...
            if request_data and request_data.get('update_type') == 'invoice_paid':
                ...
                # New format: payload is payment_id for pending transaction.
                try:
                    inv = payload_data or {}
                    amount_currency = inv.get("paid_amount")
                    currency_name = inv.get("paid_asset")
                    metadata = handlers.find_and_complete_pending_transaction(
                        payment_id=payload_string,
                        amount_rub=None,
                        payment_method="CryptoBot",
                        currency_name=currency_name,
                        amount_currency=(float(amount_currency) if amount_currency is not None else None),
                    )
                except Exception as e:
                    logger.warning(f"CryptoBot вебхук: pending-режим не сработал, fallback к legacy payload. Причина: {e}")

                # Legacy format fallback: colon-separated metadata.
                if not metadata:
                    ...
                    metadata = {
                        ...
                        "price": parts[2],
                        "action": parts[3],
                        "key_id": None if parts[4] in ('None', '') else parts[4],
                        "host_name": parts[5],
                        "plan_id": None if parts[6] in ('None', '') else parts[6],
