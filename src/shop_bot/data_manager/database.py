    ...
    try:
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            default_settings = {
                ...
                "card_holder": None,
                "card_bank": None,
                "card_transfer_instructions": None,
                "domain": None,
...
def find_and_complete_pending_transaction(
    ...
    try:
        with sqlite3.connect(DB_FILE) as conn:
            ...
            cursor.execute(
                "SELECT * FROM transactions WHERE payment_id = ? AND status IN ('pending', 'pending_review')",
                (payment_id,),
            )
...
def get_transaction_by_payment_id(payment_id: str) -> dict | None:
    try:
        ...
        logging.error(f"Не удалось получить транзакцию payment_id={payment_id}: {e}")
        return None


def update_transaction_status(payment_id: str, status: str, payment_method: str | None = None) -> bool:
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            if payment_method is None:
                ...
                cursor.execute(
                    "UPDATE transactions SET status = ?, payment_method = COALESCE(?, payment_method) WHERE payment_id = ?",
                    (status, payment_method, payment_id),
                )
            conn.commit()
            return cursor.rowcount > 0
    except sqlite3.Error as e:
        logging.error(f"Не удалось обновить статус транзакции payment_id={payment_id}: {e}")
        return False


def mark_pending_transaction_review(payment_id: str, user_id: int) -> bool:
    try:
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute(
        ...
        logging.error(f"Не удалось перевести транзакцию в pending_review payment_id={payment_id}: {e}")
        return False
